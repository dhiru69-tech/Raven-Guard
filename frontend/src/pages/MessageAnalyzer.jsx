...
function MessageAnalyzer() {
  ...
  const riskBg = result?.risk_level === "high" ? "border-red-500/80" : result?.risk_level === "medium" ? "border-amber-500/80" : "border-green-500/80";

  return (
    <MainLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          <h1 className="text-4xl font-bold text-cyan-400">Message Analyzer</h1>
          <p className="mt-2 text-slate-400">Paste any suspicious SMS, WhatsApp message, email, or notification to analyze potential fraud signals.</p>
        </div>

        <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-6">
          <label className="block mb-3 text-lg font-medium text-white">Suspicious Message</label>
          <textarea
            rows={8}
            value={text}
            onChange={(e) => setText(e.target.value)}
            placeholder="Paste suspicious content here... e.g. 'Dear customer, your SBI account has been blocked. Update KYC immediately: bit.ly/sbi-kyc'"
            className="w-full rounded-lg bg-slate-900 border border-slate-700 p-4 text-white placeholder:text-slate-500 outline-none focus:border-blue-500 transition"
          />
          <div className="mt-4 flex items-center gap-4">
            <button
              onClick={analyze}
              disabled={loading || !text.trim()}
              className="flex items-center gap-2 px-6 py-3 rounded-lg bg-cyan-500 text-black font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-cyan-400 transition"
            >
              {loading ? <><Loader2 size={18} className="animate-spin" /> Analyzing...</> : "Analyze Message"}
            </button>
            {text && <button onClick={() => { setText(""); setResult(null); setError(""); }} className="text-sm text-slate-400 hover:text-white">Clear</button>}
          </div>
        ...
        {result && (
          <>
            <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-6">
              <h2 className="text-2xl font-semibold mb-4 text-white">Threat Score</h2>
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                <div className="rounded-lg border border-slate-800 bg-slate-900 p-3">
                  ...
                  <p className="mt-1 text-sm font-semibold text-blue-400">{result.confidence}%</p>
                </div>
                <div className="rounded-lg border border-slate-800 bg-slate-900 p-3">
                  <p className="text-xs text-slate-500">Analysis Time</p>
                  <p className="mt-1 text-sm font-semibold text-blue-400">{(result.analysis_time_ms / 1000).toFixed(1)}s</p>
                </div>
                <div className="rounded-lg border border-slate-800 bg-slate-900 p-3">
                  <p className="text-xs text-slate-500">Category</p>
                  <p className="mt-1 text-sm font-semibold text-blue-400 capitalize">{result.categories?.[0] || "—"}</p>
                </div>
              </div>
            </div>
            ...
            {result.red_flags?.length > 0 && (
              <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-2xl font-semibold text-white">Red Flags Detected</h2>
                ...
                <ul className="space-y-3">
                  {result.safe_signals.map((s, i) => (
                    <li key={i} className="flex items-start gap-3 rounded-xl border border-slate-800 bg-slate-900/70 p-4">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-green-400">{s.title}</p>
